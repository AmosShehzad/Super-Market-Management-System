...
class BillingApp:
    ...
    def setup_ui(self):
        ...
        label_frame.pack()
        tk.Label(label_frame, text="Mart Land", font=("Arial", 24, "bold"), bg="lightblue").pack()
        ...
        cashmemo_button = tk.Button(tabs_frame, text="Cash Memo", bg="black", fg="white", cursor="hand2", padx=50,
                                    command=self.generate_pdf_bill)
        ...
        Bill_label = ttk.Label(bill_frame, text="Bill Number: 06", font=("Arial", 10))
    ...
    def generate_bill(self):
        ...
        cur.execute("INSERT INTO Bills (customer_name, date, total) VALUES (?, ?, ?)",
                    (name, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), total))
